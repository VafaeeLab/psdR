...
psd_without_t <- function(data){
  ...
  data.psd <- data.psd * log2(data.psd)
  data.psd <- sweep(data.psd, 1, rowMeans(data.psd))
  row_max <- apply(data.psd, 1, max)
  row_min <- apply(data.psd, 1, min)
  data.psd <- sweep(
    sweep(data.psd, 1, row_min),
    1, row_max - row_min, FUN="/"
  )
  return (data.psd)
}
